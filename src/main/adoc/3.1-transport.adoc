...
in Chrome: +
>> Developer Tools >> *Timeline* >> (Record button)

Recording of:

* *FPS* (Frame Per Second) (60 fps = 16.67 ms / frame)
* *CPU* resources: shows what type of events consumed the CPU resources
* *NET*: each bar represents an HTTP resource. The longer the bar, the slower the request-repsosne
** HTML files are blue.
** Scripts are yellow.
...
Gives an overal evalution (/ 100 points).
